...
const ProductAdminPage: FC<Props> = ({ product }) => {
  ...
  const fileInputRef = useRef<HTMLInputElement>(null)
	...
	//?para generar un slug automatico.
	useEffect(() => {
	...
	const onAddtag = (event: KeyboardEvent<HTMLDivElement>) => {
		...
		const newTagformat = newTag.toLocaleLowerCase().trim();

		if (event.code === "Space" && newTagformat.length > 0) {
			if (!getValues("tags").includes(newTagformat)) {
				setValue("tags", [...getValues("tags"), newTagformat]);
			}
	...
	const onSubmit = async (formData: FormData) => {
		if (formData.images.length < 2)
			return alert("Debe subir al menos 2 imagenes");
  ...
  const onUploadImages = async ({target}: ChangeEvent<HTMLInputElement>) => {
...
export const getServerSideProps: GetServerSideProps = async ({ query }) => {
	...
	if (slug === "new") {
		...
		product = await dbProducts.getBySlug(slug.toString());
	}
